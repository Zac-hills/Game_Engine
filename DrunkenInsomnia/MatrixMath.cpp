...
mat4 mat4::rotateY(const float degrees) const
{
	float radians;
...
mat4 mat4::rotateX(const float degrees) const
{
	float radians;
	radians = degrees * ((float)PI / 180.0f);
...
mat4 mat4::rotateZ(const float degrees) const
{
	float radians;
...
void mat4::perspectiveMatrix(const float _near, const float _far, const float FOV, const float height, const float width)
{
...
void mat4::OrthoGraphicMatrix(const float right, const float left, const float top, const float bottom, const float a_near, const float a_far)
{
	data[0] = 2/(right - left);   data[4] = 0;					data[8] = 0;					data[12] = 0;
	data[1] = 0;				  data[5] = 2/(top-bottom);     data[9] = 0;					data[13] = 0;
	data[2] = 0;				  data[6] = 0;					data[10] = -2/(a_far-a_near);   data[14] = -((a_far + a_near)/(a_far - a_near));
	data[3] = 0;				  data[7] = 0;					data[11] = 0;					data[15] = 1;
}